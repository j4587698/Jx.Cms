...
public class ResponsivePageMatcherPolicy : MatcherPolicy, IEndpointComparerPolicy, IEndpointSelectorPolicy
{
    ...
    {
        Comparer = EndpointMetadataComparer<ThemeNameAttribute>.Default;
    }
    ...
    public bool AppliesToEndpoints(IReadOnlyList<Endpoint> endpoints)
    {
        foreach (var endpoint in endpoints)
            if (endpoint?.Metadata.GetMetadata<IThemeNameMetadata>() != null)
                return true;
        return false;
    }
    ...
    public Task ApplyAsync(HttpContext httpContext, CandidateSet candidates)
    {
        ...
        for (var i = 0; i < candidates.Count; i++) candidateStates.Add(candidates[i]);
        ...
        if (path.IsNullOrEmpty()) return Task.CompletedTask;
        for (var i = 0; i < candidates.Count; i++)
        {
            var endpoint = candidates[i].Endpoint;
            var metaData = endpoint.Metadata.GetMetadata<IThemeNameMetadata>();
            if (metaData?.ThemeName != path) candidates.SetValidity(i, false);
        }
