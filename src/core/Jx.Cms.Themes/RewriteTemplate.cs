...
public static class RewriteTemplate
{
    public static List<string> CreateUrl(string url)
    {
        ...
        if (url == null) return urlList;
        var mc = Regex.Matches(url, @"\{\{(.+?)\}\}");
        foreach (Match match in mc)
        {
            urlList.Add(match.Groups[1].Value);
            if (match.Value == "{{year}}" || match.Value == "{{month}}" || match.Value == "{{day}}" ||
                match.Value == "{{id}}" || match.Value == "{{page}}")
    ...
    {
        if (urlList.Count < 2) return (false, null);

        var mc = Regex.Match(baseUrl, urlList[0]);
        if (!mc.Success || mc.Groups.Count != urlList.Count) return (false, null);

        var result = new Dictionary<string, string>();
        for (var i = 1; i < mc.Groups.Count; i++) result.Add(urlList[i], HttpUtility.UrlDecode(mc.Groups[i].Value));
