...
public class MyViewCompiler : IViewCompiler
{
    ...
    public MyViewCompiler(
        ...
        ILogger logger)
    {
        if (compiledViews == null) throw new ArgumentNullException(nameof(compiledViews));
    ...
    public Task<CompiledViewDescriptor> CompileAsync(string relativePath)
    {
        ...
        if (_compiledViews.TryGetValue(relativePath, out var cachedResult)) return cachedResult;

        var normalizedPath = GetNormalizedPath(relativePath);
        if (_compiledViews.TryGetValue(normalizedPath, out cachedResult)) return cachedResult;
    ...
    public static string NormalizePath(string path)
    {
        ...
        if (!addLeadingSlash && !transformSlashes) return path;

        var length = path.Length;
        if (addLeadingSlash) length++;
        ...
        return string.Create(length, (path, addLeadingSlash), (span, tuple) =>
        {
            ...
            foreach (var ch in pathValue) span[spanIndex++] = ch == '\\' ? '/' : ch;
        });
    }
}