...
{
    private static readonly Dictionary<ThemeType, PluginLoader> PluginLoaders = new();
        ...
        Assembly pluginAssembly)
    {
        ...
        foreach (var attr in relatedAssembliesAttrs)
        {
            ...
            foreach (var part in partFactory.GetApplicationParts(assembly)) partManager.ApplicationParts.Add(part);
        }
    }
    ...
    {
        if (!PluginLoaders.TryGetValue(themeType, out var plugin)) return;
        plugin.Reload();
    }
}