...
public class TagService : ITagService
{
    ...
    {
        return ArticleEntity.Select.Where(x => x.Tags.AsSelect().Any(y => y.Id == id)).Page(pageNumber, pageSize)
    ...
    {
        return ArticleEntity.Select.Where(x => x.Tags.AsSelect().Any(y => y.Id == id)).Count(out count)
