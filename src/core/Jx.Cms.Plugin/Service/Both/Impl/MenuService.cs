...
public class MenuService : IMenuService
{
    ...
    {
        return MenuEntity.Where(x => x.ParentId == 0 && x.MenuName == menuName).AsTreeCte()
            .OrderByDescending(x => x.Order).ToTreeList();
    }
    ...
    {
        return MenuEntity.Where(x => x.ParentId == parentId && x.MenuName == menuName).OrderByDescending(x => x.Order)
            .IncludeMany(x => x.Children).ToList();
    }
    ...
    public bool SaveMenu(IEnumerable<MenuEntity> menus)
    {
        ...
        {
            foreach (var menuEntity in menus) menuEntity.Save();
        });
        return true;
    }
