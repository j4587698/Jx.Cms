...
public class PageService : IPageService
{
    ...
    {
        return ArticleEntity.Select.Where(x => x.IsPage).Count(out count).OrderByDescending(x => x.Id)
            .Page(pageNumber, pageSize).ToList();
    }

    public List<ArticleEntity> GetPageWithPage(int pageNumber, int pageSize)
    {
        return ArticleEntity.Select.Where(x => x.IsPage).OrderByDescending(x => x.Id).Page(pageNumber, pageSize)
            .ToList();
    }
