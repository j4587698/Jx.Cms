...
public class CatalogService : ICatalogService
{
    ...
    public List<ArticleEntity> GetArticlesByCatalogId(int id, bool includeChildren, int pageNumber, int pageSize,
        out long count)
    {
        var catalogues = new List<int>();
        if (includeChildren)
            catalogues = CatalogEntity.Where(x => x.Id == id).AsTreeCte().ToList(x => x.Id);
        else
            catalogues.Add(id);

        return ArticleEntity.Where(x =>
                x.Status == ArticleStatusEnum.Published && x.IsPage == false && catalogues.Contains(x.CatalogueId))
