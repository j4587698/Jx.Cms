...
public class PaginationService : IPaginationService
{
    public Dictionary<string, long> GetPagination(int pageNo, int pageSize, long totalCount)
    {
        ...
        if (pageNo != 1) result.Add("<<", 1);
        if (pageNo > 1) result.Add("<", pageNo - 1);
        for (var i = startNo; i <= pageNo + 5 && i <= totalPage; i++) result.Add(i.ToString(), i);
        if (pageNo < totalPage) result.Add(">", pageNo + 1);
        if (pageNo != totalPage) result.Add(">>", totalPage);
        return result;
    }
}