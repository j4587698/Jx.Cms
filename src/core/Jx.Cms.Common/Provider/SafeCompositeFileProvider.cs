    ...
    public class SafeCompositeFileProvider : IFileProvider
    {
        ...
        public IFileInfo GetFileInfo(string subpath)
        {
            ...
            foreach (var provider in _fileProviders)
            {
                ...
                {
                    // 对于其他异常，记录并重新抛出
                    System.Diagnostics.Debug.WriteLine($"Error getting file info for {subpath}: {ex.Message}");
                    throw;
                }
            }
        ...
        public IDirectoryContents GetDirectoryContents(string subpath)
        {
            ...
            if (_fileProviders.Count == 1)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Error getting directory contents for {subpath}: {ex.Message}");
                    throw;
                }
            }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error creating composite directory contents for {subpath}: {ex.Message}");
                throw;
            }
        }
        ...
        public IChangeToken Watch(string pattern)
        {
            ...
            foreach (var provider in _fileProviders)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Error watching pattern {pattern}: {ex.Message}");
