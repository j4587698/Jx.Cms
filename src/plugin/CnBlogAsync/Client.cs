...
using Array = CnBlogAsync.XmlRPC.Array;
...
public class Client
{
    ...
    public string NewPost(string title, string description, IList<string> categories, DateTime? date_created,
        bool publish = true)
    {
        Array cats = null;
        ...
        {
            cats = new Array(categories.Count);
    ...
    public bool EditPost(string postid, string title, string description, IList<string> categories, bool publish)
    {
        ...
        var categories_ = new Array(categories == null ? 0 : categories.Count);
    ...
    public int NewCategory(WpCategory wpCategory)
    {
        ...
        if (!wpCategory.Slug.IsNullOrEmpty()) struct_["slug"] = new StringValue(wpCategory.Slug);

        if (!wpCategory.Description.IsNullOrEmpty()) struct_["description"] = new StringValue(wpCategory.Description);
